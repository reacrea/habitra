...
import { a as CONTACT_INTEREST_TYPES, i as CONTACT_AVAILABILITY, n as scheduleVisitSchema, o as CONTACT_PREFERRED_METHODS, r as startBuyingProcessSchema, t as contactAgentSchema } from "./public-engagement-B31VPc5E.js";
import { t as createServerRpc } from "./createServerRpc-DaHnQtmK.js";
import { n as getOrganizationScope, t as assertOrganizationAccess } from "./org-access-BTjeUve1.js";
...
*/
async function assertPublicEngagementOrgAccess(userId, role, propertyOrganizationId) {
	if (role === "BUYER" || role === "SELLER") return;
	assertOrganizationAccess(await getOrganizationScope(userId, role), propertyOrganizationId);
}
/**
...
*/
async function resolveListingAgentUserIdOrNull(organizationId, propertyAssignedAgentId) {
	if (!propertyAssignedAgentId) return null;
	return await prisma.membership.findFirst({ where: {
		organizationId,
		userId: propertyAssignedAgentId
	} }) ? propertyAssignedAgentId : null;
}
/**
...
*/
async function resolveAssignableAgentUserId(organizationId, propertyAssignedAgentId) {
	...
	memberships.sort((a, b) => {
		return (MEMBERSHIP_AGENT_PRIORITY[a.role] ?? 99) - (MEMBERSHIP_AGENT_PRIORITY[b.role] ?? 99);
	});
	const first = memberships[0];
	if (!first) throw new Response("La propiedad no tiene agente asignado y la inmobiliaria no tiene agentes o responsables configurados.", { status: 409 });
	return first.userId;
}
...
var contactAgentFromPublic = createServerFn({ method: "POST" }).inputValidator((data) => contactAgentSchema.parse(data)).handler(contactAgentFromPublic_createServerFn_handler, async ({ data }) => {
	...
	const listingAgentId = await resolveListingAgentUserIdOrNull(property.organizationId, property.assignedAgentId);
	...
	const displayName = [buyer?.name, user.name].find((n) => typeof n === "string" && n.trim().length >= 2)?.trim() ?? user.email?.split("@")[0]?.trim() ?? "Contacto";
	const email = buyer?.email ?? user.email ?? null;
	const phone = buyer?.phone ?? user.phone ?? null;
	const interestLabel = CONTACT_INTEREST_TYPES.find((x) => x.value === data.interestType)?.label ?? data.interestType;
	const availabilityLabel = CONTACT_AVAILABILITY.find((x) => x.value === data.availability)?.label ?? data.availability;
	const methodLabel = CONTACT_PREFERRED_METHODS.find((x) => x.value === data.preferredContactMethod)?.label ?? data.preferredContactMethod;
	const noteLines = [];
	if (data.message?.trim()) noteLines.push(data.message.trim());
	noteLines.push(`Tipo de interes: ${interestLabel}`, `Disponibilidad: ${availabilityLabel}`, `Metodo preferido: ${methodLabel}`, "", `Propiedad de interes: ${property.title} (${property.slug})`);
