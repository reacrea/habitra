...
function getURL(url, options) {
	...
	try {
		if (url.startsWith("http")) _url = url;
			...
			if (baseURL && !(baseURL == null ? void 0 : baseURL.endsWith("/"))) baseURL = baseURL + "/";
			if (url.startsWith("/")) _url = new URL(url.substring(1), baseURL);
			else _url = new URL(url, options == null ? void 0 : options.baseURL);
		}
	} catch (e) {
		if (e instanceof TypeError) {
			if (!(options == null ? void 0 : options.baseURL)) throw TypeError(`Invalid URL ${url}. Are you passing in a relative url but not setting the baseURL?`);
			throw TypeError(`Invalid URL ${url}. Please validate that you are passing the correct input.`);
		}
		throw e;
	}
	if (options == null ? void 0 : options.params) if (Array.isArray(options == null ? void 0 : options.params)) {
		const params = (options == null ? void 0 : options.params) ? Array.isArray(options.params) ? `/${options.params.join("/")}` : `/${Object.values(options.params).join("/")}` : "";
		_url = _url.toString().split("/:")[0];
		_url = `${_url.toString()}${params}`;
	} else for (const [key, value] of Object.entries(options == null ? void 0 : options.params)) _url = _url.toString().replace(`:${key}`, String(value));
...
//# sourceMappingURL=@better-fetch_fetch.js.map